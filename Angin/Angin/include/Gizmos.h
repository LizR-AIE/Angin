...
public:
	...
	ANGIN_API static void		create(unsigned int a_maxLines = 0xffff, unsigned int a_maxTris = 0xffff,
						   unsigned int a_max2DLines = 0xff, unsigned int a_max2DTris = 0xff);
	ANGIN_API static void		destroy();

	// removes all Gizmos
	ANGIN_API static void		clear();

	// draws current Gizmo buffers, either using a combined (projection * view) matrix, or separate matrices
	ANGIN_API static void		draw(const glm::mat4& a_projectionView);
	ANGIN_API static void		draw(const glm::mat4& a_projection, const glm::mat4& a_view);
	
	// the projection matrix here should ideally be orthographic with a near of -1 and far of 1
	ANGIN_API static void		draw2D(const glm::mat4& a_projection);

	// Adds a single debug line
	ANGIN_API static void		addLine(const glm::vec3& a_rv0, const glm::vec3& a_rv1,
	...
	// Adds a triangle.
	ANGIN_API static void		addTri(const glm::vec3& a_rv0, const glm::vec3& a_rv1, const glm::vec3& a_rv2, const glm::vec4& a_colour);
	...
	ANGIN_API static void		addTransform(const glm::mat4& a_transform, float a_fScale = 1.0f);
	
	// Adds a wireframe Axis-Aligned Bounding-Box with optional transform for rotation/translation.
	ANGIN_API static void		addAABB(const glm::vec3& a_center, const glm::vec3& a_extents,
	...
	ANGIN_API static void		addAABBFilled(const glm::vec3& a_center, const glm::vec3& a_extents,
	...
	ANGIN_API static void		addCylinderFilled(const glm::vec3& a_center, float a_radius, float a_fHalfLength,
	...
	ANGIN_API static void		addRing(const glm::vec3& a_center, float a_innerRadius, float a_outerRadius,
							unsigned int a_segments, const glm::vec4& a_fillColour, const glm::mat4* a_transform = nullptr);
	...
	ANGIN_API static void		addDisk(const glm::vec3& a_center, float a_radius,
							unsigned int a_segments, const glm::vec4& a_fillColour, const glm::mat4* a_transform = nullptr);

	// Adds an arc, around the Y-axis
	// If a_rvFilLColour.w == 0 then only an outer line is drawn.
	ANGIN_API static void		addArc(const glm::vec3& a_center, float a_rotation,
	...
	// If a_rvFilLColour.w == 0 then only an outer line is drawn.
	ANGIN_API static void		addArcRing(const glm::vec3& a_center, float a_rotation,
	...
	ANGIN_API static void		addSphere(const glm::vec3& a_center, float a_radius, int a_rows, int a_columns, const glm::vec4& a_fillColour,
	...
	ANGIN_API static void		addHermiteSpline(const glm::vec3& a_start, const glm::vec3& a_end,
	...
	ANGIN_API static void		add2DLine(const glm::vec2& a_start, const glm::vec2& a_end, const glm::vec4& a_colour);
	ANGIN_API static void		add2DLine(const glm::vec2& a_start, const glm::vec2& a_end, const glm::vec4& a_colour0, const glm::vec4& a_colour1);
	ANGIN_API static void		add2DTri(const glm::vec2& a_0, const glm::vec2& a_1, const glm::vec2& a_2, const glm::vec4& a_colour);
	ANGIN_API static void		add2DAABB(const glm::vec2& a_center, const glm::vec2& a_extents, const glm::vec4& a_colour, const glm::mat4* a_transform = nullptr);
	ANGIN_API static void		add2DAABBFilled(const glm::vec2& a_center, const glm::vec2& a_extents, const glm::vec4& a_colour, const glm::mat4* a_transform = nullptr);
	ANGIN_API static void		add2DCircle(const glm::vec2& a_center, float a_radius, unsigned int a_segments, const glm::vec4& a_colour, const glm::mat4* a_transform = nullptr);
